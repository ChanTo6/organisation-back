...
namespace project.packages
{
    ...
    public interface IPKG_TO_DO
    {
        ...
        public Task UpdateUserByPersonId(int personId, string employeeName, string employeeSurname, string password, string role, decimal telephone, string orgName);
        ...
        public Task AddProductToWarehouse(string productName, int quantity, int userId);
        public Task EditProductInWarehouse(int productId, string productName, int quantity, string barcode, int userId);
        public  Task<List<Product>> FetchProductbyuserId(int userId);
        public  Task<List<Product>> FetchProducts();
        public Task<List<Product>> FetchTakenProducts();
    ...
    public class PKG_TO_DO : PKG_BASE, IPKG_TO_DO
    {
        ...
        public async Task UpdateUserByPersonId(int personId, string employeeName, string employeeSurname, string password, string role, decimal telephone, string orgName)
        {
            try
            {
                using (OracleConnection conn = new OracleConnection(Connstr))
                {
                    ...
                    using (OracleCommand cmd = new OracleCommand("PROJECT_pkg.UPDATE_USER_BY_PERSON_ID", conn))
                    {
                        ...
                        cmd.Parameters.Add("p_role", OracleDbType.Varchar2).Value = role;
                        cmd.Parameters.Add("p_telephone", OracleDbType.Int32).Value = telephone;
            ...
            List<Claim> claims = new List<Claim>
    {
        ...
        public async Task EditProductInWarehouse(int productId, string productName, int quantity, string barcode, int userId)
        {
            try
            {
                using (var conn = new OracleConnection(Connstr))
        ...
        public async Task<List<Product>> FetchTakenProducts()
        {
            var products = new List<Product>();
            try
            {
                using (var conn = new OracleConnection(Connstr))
                {
                    ...
                    using (var cmd = new OracleCommand("PROJECT_pkg.GetTakenProducts", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public async Task AddProductToWarehouse(string productName, int quantity, int userId)
        {
            string barcode;
